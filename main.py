...
def format_response_data(data, platform, url):
    ...
    try:
        ...
        author_name = author_info.get('unique_id', 'unknown')
...
def handle_extract_urls(data):
    ...
    if not urls:
        emit('extraction_error', {'error': 'Không có URL để xử lý'})
        return
    
    # Gửi thông báo bắt đầu
    emit('extraction_start', {
        'total': len(urls),
        'message': f'Bắt đầu trích xuất {len(urls)} link'
    })
    
    # Thực hiện extraction async
    def process_urls():
        ...
        try:
            ...
            emit('extraction_error', {
                'error': f'Lỗi khi trích xuất: {str(e)}'
            })
        finally:
            emit('extraction_complete', {'message': 'Hoàn thành trích xuất'})
            loop.close()
